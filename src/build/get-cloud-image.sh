...
images=$(aws ec2 describe-images --output json \
  --region us-east-1 --filters "Name=name,Values=appbricks-bastion-inceptor*")

if [[ $1 == prod ]]; then
  prod_images=$(echo "$images" | jq '[.Images[] | select(.Name|test("appbricks-bastion-inceptor_\\d+\\.\\d+\\.\\d+"))'])
  appbricks_cloud_image=$(echo "$prod_images" | jq -r 'sort_by(.Name | split("_")[1] | split(".") | map(tonumber))[-1] | .Name')

else
  dev_images=$(echo "$images" | jq '[.Images[] | select(.Name|test("appbricks-bastion-inceptor_D.*"))]')
